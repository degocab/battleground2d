    ...
    private void Update()
    {
        ...
        if (entityManager.TryGetSingleton<GameStateComponent>(out var gameState))
        {
            if (gameState.CurrentState == GameState.Playing)
            {
                unitFactory = new UnitFactory(entityManager);
                //unitFactory.SpawnUnits(spawnConfig.UnitCountToSpawn);
                unitFactory.SpawnUnits(100, UnitType.Default, Direction.Left, CommandFactory.CreateIdleCommand(), new float2(7, 0), FormationType.Phalanx);
                unitFactory.SpawnUnits(100, UnitType.Enemy, Direction.Right, CommandFactory.CreateIdleCommand(), new float2(-5, 0), FormationType.Horde);
                unitFactory.SpawnCommander();
