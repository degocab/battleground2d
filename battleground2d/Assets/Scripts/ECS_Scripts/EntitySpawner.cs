        ...
        if (entityManager.TryGetSingleton<GameStateComponent>(out var gameState))
        {
            if (gameState.CurrentState == GameState.Playing)
            {
                ...
                unitFactory.SpawnUnits(200
                    , UnitType.Enemy, Direction.Right, CommandFactory.CreateIdleCommand(), new float2(-3, 0), FormationType.Horde);
                unitFactory.SpawnUnits(200
                    , UnitType.Enemy, Direction.Right, CommandFactory.CreateIdleCommand(), new float2(-3, -5), FormationType.Horde);
