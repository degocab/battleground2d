...
namespace RTSToolkit
{
    public class KDTree2D
    {
        ...
        public static KDTree2D MakeFromPointsGo(List<GameObject> pointsGo)
        {
            Vector2[] pos = new Vector2[pointsGo.Count];
            ...
            KDTree2D kd = MakeFromPoints(pos);
            return kd;
        }

        //	Make a new tree from a list of points.
        public static KDTree2D MakeFromPoints(params Vector2[] points)
        {
        ...
        static KDTree2D MakeFromPointsInner(
            int depth,
        ...
        static int FindSplitPoint(Vector2[] points, int[] inds, int stIndex, int enIndex, int axis)
        {
        ...
        //	of its plane.
        public static int FindPivotIndex(Vector2[] points, int[] inds, int stIndex, int enIndex, int axis)
        {
            int splitPoint = FindSplitPoint(points, inds, stIndex, enIndex, axis);

            Vector2 pivot = points[inds[splitPoint]];
            ...
            while (currPt <= endPt)
            {
                Vector2 curr = points[inds[currPt]];
        ...
        public int FindNearest(Vector2 pt)
        {
        ...
        public float FindNearest_R(Vector2 pt)
        {
            float bestSqDist = float.MaxValue;
            int bestIndex = -1;
        ...
        void Search(Vector2 pt, ref float bestSqSoFar, ref int bestIndex)
        {
        ...
        float DistFromSplitPlane(Vector2 pt, Vector2 planePt, int axis)
        {
            return pt[axis] - planePt[axis];
        }

        // Find and returns	k-th nearest neighbour
        public int FindNearestK(Vector2 pt, int k)
        {
            float bestSqDist = float.MaxValue;
            float minSqDist = 0f;
            int bestIndex = -1;
        ...
        public int[] FindNearestsK(Vector2 pt, int k)
        {
            float bestSqDist = float.MaxValue;
            float minSqDist = 0f;
            int bestIndex = -1;
        ...
        public float FindNearestK_R(Vector2 pt, int k)
        {
            float bestSqDist = float.MaxValue;
            float minSqDist = 0f;
            int bestIndex = -1;
        ...
        public float[] FindNearestsK_R(Vector2 pt, int k)
        {
            float bestSqDist = float.MaxValue;
            float minSqDist = 0f;
            int bestIndex = -1;
        ...
        public int[] FindNearestsR(Vector2 pt, float rcrit)
        {
            int k = 2;
            int[] pts = null;
        ...
        void SearchK(Vector2 pt, ref float bestSqSoFar, ref float minSqDist, ref int bestIndex)
        {
            float mySqDist = (pivot - pt).sqrMagnitude;
