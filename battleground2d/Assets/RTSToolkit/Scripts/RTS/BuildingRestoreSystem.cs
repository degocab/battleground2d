...
{
    public class BuildingRestoreSystem : MonoBehaviour
    {
        public static BuildingRestoreSystem active;
        ...
        void UpdateBuilding(UnitPars up)
        {
            if ((up.isDying == false) && (up.isSinking == false))
            {
                ...
                if (multiplayerPass)
                {
                    ...
                    ReduceCostsFromEconomyAndUpdateHealth(up, healAmount);
                }
            }
            else
            {
        ...
        void ReduceCostsFromEconomyAndUpdateHealth(UnitPars up, float healAmount)
        {
            int nation = up.nation;
            UnitParsType unitParsType = up.unitParsType;

            int[] resourcesToRemove = new int[unitParsType.costs.Count];

            for (int i = 0; i < unitParsType.costs.Count; i++)
            {
                resourcesToRemove[i] = GenericMath.FloatToIntRandScaled(0.5f * (healAmount / up.maxHealth) * unitParsType.costs[i].amount);
            }

            bool economyPass = true;

            for (int i = 0; i < Economy.active.nationResources[nation].Count; i++)
            {
                for (int j = 0; j < unitParsType.costs.Count; j++)
                {
                    if (unitParsType.costs[j].name == Economy.active.nationResources[nation][i].name)
                    {
                        if ((Economy.active.nationResources[nation][i].amount - resourcesToRemove[j]) <= 0)
                        {
                            economyPass = false;
                        }
                    }
            ...
            if (economyPass)
            {
                for (int i = 0; i < Economy.active.nationResources[nation].Count; i++)
                {
                    for (int j = 0; j < unitParsType.costs.Count; j++)
                    {
                        if (unitParsType.costs[j].name == Economy.active.nationResources[nation][i].name)
                        {
                            Economy.active.nationResources[nation][i].amount = Economy.active.nationResources[nation][i].amount - resourcesToRemove[j];
                        }
                    }
                }
