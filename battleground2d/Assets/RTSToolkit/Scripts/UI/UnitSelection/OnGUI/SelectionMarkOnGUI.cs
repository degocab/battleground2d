...
{
    public class SelectionMarkOnGUI : MonoBehaviour
    {
        public static SelectionMarkOnGUI active;
        ...
        void OnGUI()
        {
            if (unlocked == true)
            {
                ...
                {
                    UnitPars up = SelectionManager.active.selectedGoPars[i];

                    screenPos = camera.WorldToScreenPoint(up.transform.position + up.unitParsType.unitCenter);
                    Vector3 heading = up.transform.position + up.unitParsType.unitCenter - camPos;

                    float unitSize = up.unitParsType.unitSize;
